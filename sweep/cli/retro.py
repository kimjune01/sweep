...
def retro_reclassify(
    dry_run: bool = typer.Option(
        False, "--dry-run",
        help="Only show what would be kicked; don't deposit cards.",
    ),
) -> None:
    """Walk every hypothesis-graph artifact and kick a switch card for
    each. switch (the central decision actor) does the classification
    and routing — this CLI is the bulk re-entry point after a prompt
    change or backfill. Cards dedupe at the actor; safe to re-run."""
    import asyncio as _asyncio
    from pathlib import Path as _Path

    from sweep.activities.switch import kick_switch_card

    HG_DIR = _Path("/Users/junekim/Documents/sweep/repo-hypotheses")
    if not HG_DIR.exists():
        print(f"_no hypothesis-graph directory at {HG_DIR}_")
        return

    targets: list = []
    for p in HG_DIR.glob("*.md"):
        stem = p.stem
        parts = stem.split("__")
        if len(parts) < 3:
            continue  # per-repo (no issue number) format, skip
        try:
            issue = int(parts[-1])
        except ValueError:
            continue
        owner = parts[0]
        repo_name = "__".join(parts[1:-1])
        targets.append((f"{owner}/{repo_name}", issue, str(p)))

    print(f"# Reclassify ({len(targets)} hygraph artifacts)")
    print()
    if dry_run:
        print(f"_dry-run: {len(targets)} switch card(s) would be kicked_")
        return

    async def _kick_all() -> int:
        n = 0
        for repo, issue, art in targets:
            try:
                await kick_switch_card(
                    repo, issue, artifact_path=art, source="reclassify",
                )
                n += 1
            except Exception as e:
                observe.event("reclassify_kick_failed", repo=repo, issue=issue,
                              error_type=type(e).__name__, error=str(e)[:200])
        return n

    kicked = _asyncio.run(_kick_all())
    print(f"_kicked {kicked}/{len(targets)} switch cards; "
          f"watch via `sweep observe events --kind switch_done`_")
