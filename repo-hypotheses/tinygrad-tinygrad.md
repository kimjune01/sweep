...
**Active until 2026-05-22.** geohot on PR #16113: "You are close to getting banned. A PR means I have spent serious human time reading this and 100% believe you are ready to just click merge." All 10 PRs (#16094–#16117) closed, none merged. No submissions until cooldown expires.

### Your items

| # | Type | Score | Signal | Title | Status |
|---|------|-------|--------|-------|--------|
| 16113 | PR | 4 | Your PR, ban warning from geohot | Failing tests (MATVEC + PTX bf16) | CLOSED |
| 16114 | Issue | 3 | Issue you filed, tagged geohot | Two bugs: MATVEC + PTX bfloat16 | CLOSED |
...
## T12296: max backward can underflow (float16)

**Status:** CONFIRMED. Fix verified, all tests pass.

### Root cause

`gradient.py` line 12: MAX backward casts the boolean mask to `ctx.dtype` (half) before counting. 70000 exceeds float16 max (65504), count overflows to inf, `1/inf = 0` kills every gradient.

### Fix (net-zero lines, zero new imports)

```python
# gradient.py line 12-14. Two token-level changes:
mask = ret.src[0].eq(broadcast_to_input(ret)).cast(sum_acc_dtype(ctx.dtype))  # was: .cast(ctx.dtype)
count = mask._rop(Ops.ADD, ret.arg[1])
return ((mask/broadcast_to_input(count)).cast(ctx.dtype) * broadcast_to_input(ctx),)  # added: .cast(ctx.dtype)
```

Uses `sum_acc_dtype` (already imported line 5, same pattern as EXPAND backward line 70). Accumulates in float32, casts back.
...
def test_max_backward_half(self):
  ...
  t.max().backward()
  assert np.isfinite(t.grad.numpy()).all() and t.grad.numpy().sum() > 0.5
```

### PR viability: STRONG

Net-zero lines, zero imports, follows existing pattern, addresses chenyuxyz's #13322 feedback ("can it be more fundamental?"). Supersedes stale PR #13322 (uses old API).

---

## T6909: bf16 autocast — ClangRenderer emits non-portable `__bf16` type

**Status:** CONFIRMED. Fix verified, test approach validated.

### Root cause

`ClangRenderer` at `cstyle.py` line 230 has no `bfloat16` in `type_map`. Falls back to `__bf16` C type, which fails on clang < 16 / x86 targets. ALU ops are already decomposed via `pm_manual_bf16_cast`, but the type declaration persists.

### Fix (+4 lines)

```python
# cstyle.py ClangRenderer, line 230:
type_map = {dtypes.bool:"_Bool", dtypes.half:"__fp16", dtypes.bfloat16:"unsigned short"}  # added bf16

# Add string_rewrite for bf16 CONST (after line 230):
...
Same pattern as OpenCL renderer (lines 294-302).

### Test (fails on master)

```python
def test_bf16_no_native_type(self):
  from tinygrad.renderer.cstyle import ClangRenderer
  ast = (Tensor.empty(4, dtype=dtypes.bfloat16).float()).schedule_linear().src[-1].src[0]
  r = ClangRenderer(Target(device="CPU", arch="x86_64"))
  p = to_program(ast, r)
  self.assertNotIn("__bf16", p.src[3].arg)
```

### PR viability: OK

+4 lines (not net deletion), but geohot filed this issue, prior PR #15718 had code approval (only "needs regression test" objection — this test addresses that). Two prior PRs stalled (#15004, #15718) — fresh submission with test may succeed.

---

## T7020: TinyJit returns wrong values (same-object aliasing)

**Status:** CONFIRMED. Root cause clear, fix has trade-offs.

### Root cause

`CapturedJit.__call__` (jit.py line 199-203) returns `self.ret` — the identical Python object every call. Buffer is reused. `r1 is r2` is literally True. Compiler constant-folds `x - x` to `0.0f`.

Cross-ref: #6803 (SDXL SplitVanillaCFG) is the exact same bug.

### Fix (+10 lines)

Clone output tensors after `run_linear` so each call returns independent copies. Adds `_clone_ret` helper.

### PR viability: BLOCKED

Net +10 lines, adds runtime overhead (buffer copy per JIT call). Violates "never trade complexity for speed" in reverse — adds complexity for correctness. The tinygrad team may prefer a different approach (opt-in cloning, scheduler-level fix). Not submitting during cooldown.

---

## T13179: Variable expression equality bug

**Status:** CONFIRMED. Fix verified, 351 tests pass.

### Root cause

`symbolic.py` has no reassociation rule to push outer constants through multiply chains into inner ADDs. `((a+6)*(a+12))*22` and `(a+6)*(a*22+264)` don't normalize to the same UOp.

### Fix (+1 line)

```python
# symbolic.py, near line 236 (alongside existing distribution rules):
((UPat.var("x") * (UPat.var("y", dtype=dtypes.weakint) + UPat.cvar("c1"))) * UPat.cvar("c2"),
 lambda x, y, c1, c2: x * (y * c2 + c1 * c2)),
```

Combines reassociation + distribution in one step. Non-overlapping with existing rule 272.

### Test (fails on master)

```python
def test_variable_expression_equality(self):
    a = Variable("a", 0, 1)
    b = (a+6)*(a*22+264)
    c = (a+6)*(a+12)*22
    assert b.simplify() is c.simplify()
```

### PR viability: OK

+1 line, algebraic identity, but not net deletion. Solves specific case only, not general polynomial equality. 6-month-old issue with 1 comment.

---

## T13409: ScatterND infinite loop in graph_rewrite

**Status:** IN_PROGRESS. Reproduces (bug is in data/index size causing graph explosion, not a loop). Investigation ongoing — the scatter one-hot approach blows up for large data. Fix requires either a new scatter primitive or a chunked-realize workaround.
