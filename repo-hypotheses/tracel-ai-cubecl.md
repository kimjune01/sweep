...
- Root cause: Copy-paste bug in optimizer visitor. `CompareAndSwap` visited `op.cmp` twice, never visited `op.input`.
...
1. **#1283 first**: Simplest fix, reporter gave exact line. No controversy.
2. **#1318 second**: One-line copy-paste bug fix. Clear root cause.
3. **#1316 last**: Largest change (19 call sites). Maintainers may prefer polyfill over compile-time error.
